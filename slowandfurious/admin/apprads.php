...
if (!isset($_REQUEST['adis'])) {
    echo '<h3>List of advertistment request</h3>';
} else {
    echo '<h3>List of advertistment request: ' . $_REQUEST['adis'] . '</h3>';
}
?>
                    <table class="table table-black table-hover table-striped w-75">
                        ...
                        <tbody>
                            <?php
if (!isset($_REQUEST['adis'])) {
    $stmt = $pdo->query("SELECT * FROM advertform a INNER JOIN organisation o ON a.or_id = o.or_id WHERE a.status = 0");
} else {
...
href="approve.php?adis=' . $rows['ad_id'] . '">Approve ads</a> </td> ');
echo ("</tr>");
}
?>
                        </tbody>
